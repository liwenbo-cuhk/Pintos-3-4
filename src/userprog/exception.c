...
page_fault (struct intr_frame *f)
{
    ...
    if (valid_page(fault_addr, f))
    {
       ...
       /* Need to reclaim a page. */
       if (pagedir_get_page (thread_current ()->pagedir, upage) == NULL)
        {
           is_load = virtual_memory_page_load (thread_current ()->supplement_table, thread_current ()->pagedir, upage);            
           success = true;
        }
        /* Grow the stack.*/
        while (!is_load && upage < PHYS_BASE && pagedir_get_page (thread_current ()->pagedir, upage) == NULL)
        {
          ...
          success = pagedir_set_page (thread_current ()->pagedir, upage, kpage, writable);
          if (!success)
            break;
          upage += PGSIZE;    
        }
