  ...
  for (i = 0; i < howmany; i++)
    {
      ...
      if (i == 0)
        ...
        fail ("after run %d/%d, expected depth %d.",
              i, howmany, expected_depth);
  ...
  if (n == 0)
    {
      if (expected_depth < EXPECTED_DEPTH_TO_PASS)
        fail ("should have forked at least %d times. actual %d times", EXPECTED_DEPTH_TO_PASS, expected_depth);
